...
export class DataReader {
  ...
  readNUMBER(byteLength: TwincatDatatype) : number {
    let result = 0
    let charIdx = 0;
    if (byteLength == 10) {byteLength = 1} //Special case for booleans
    ...
    this.logger.trace("DataReader.readNUMBER("+byteLength+"):"+this.convertToInt(result,byteLength));
    return this.convertToInt(result,byteLength);
    //return result
  }

  readREAL() : number {
    this.logger.trace("DataReader.readREAL")
    let data:number[] = [];
  ...
  readLREAL() : number {
    this.logger.trace("DataReader.readLREAL");
    let data = []
  ...
  private convertToInt(value:number, byteLength:TwincatDatatype): number {
    this.logger.trace("DataReader.convertToInt("+value+","+byteLength+")");
